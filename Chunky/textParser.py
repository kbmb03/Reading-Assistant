...
os.environ['GOOGLE_APPLICATION_CREDENTIALS'] = r'' #add the path of the json credential file which is downloaded 
...
def extract_text_from_pdf(pdf_path):
    """
    Extracts text from a single PDF and saves it to the specified output folder.
    
    Args:
        pdf_path (str): Path to the PDF file.
        output_folder (str): Path to the folder where the output .txt will be saved.
    """
    output_folder = r""
    if not os.path.exists(output_folder):
        os.makedirs(output_folder)
        print("created outut folder")
    process_pdf(pdf_path, output_folder)
